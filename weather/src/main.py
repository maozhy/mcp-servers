...
async def get_weather(
    city: str, units: str = "metric", lang: str = "zh_cn"
) -> WeatherData:
    """
    ...
    # 设置请求参数
    params = {"q": english_city, "appid": api_key, "units": units, "lang": lang}
...
async def get_weather_forecast(
    city: str, days: int = 5, units: str = "metric", lang: str = "zh_cn"
) -> dict:
    """
            ...
            for date, daily in list(forecasts.items())[:days]:
                result.append(
                    ForecastData(
                        ...
                        wind_speed=round(
                            sum(daily["wind_speed"]) / len(daily["wind_speed"]), 2
                        ),
                        city=city,
                    )
                )
